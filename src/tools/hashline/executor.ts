...
import { applyEdit, type EditOp, parseEditOperation } from "./edit-operations";
import { formatHashLines } from "./hash-computation";
import { HashlineMismatchError, validateLineRef } from "./validation";
 ...
 */
export async function executeHashlineEdits(content: string, payload: HashlineEditPayload): Promise<HashlineEditResult> {
  try {
    ...
    const diff = generateHashlineDiff(originalLines.join("\n"), lines.join("\n"), payload.filePath);
